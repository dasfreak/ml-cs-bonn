...
public class TdidtAlgo {
	...
	public void initTree() {
		...
		Attribute[][] attributes = DataStructureSingleton.getInstance1(); // get
																			// the
																			// array
																			// read
																			// from
		...
		Node target = new Node(trgColumn,
				...
				DataStructureSingleton.getNodeNames()[trgColumn], null);
		...
		for (int i = 0; i < FileReader.pattern.length - 1; i++) {
			...
			System.out.println("Information_Gain{"
					+ FileReader.nodesNames[currentNode.index] + "} = "
		...
		System.out.println("==> Highest Information_Gain{"
				+ FileReader.nodesNames[nodeHighestEntropy.index] + "} = "
	...
	void calcTree(Node parentNode, Attribute[][] attributes, double range,
		...
		for (Subset parentSubset : parentNode.subsets) { // iterate over all
			...
			System.out.println("Filtering on: {"
					+ FileReader.nodesNames[parentNode.index] + "->"
			...
			if (parentSubset.entropy == 0) {
				...
				parentNode.addChild(new Node(0, t, DataStructureSingleton
						.getNodeNames()[parentNode.index], parentNode));
				continue;
			}
			...
			for (int j = 0; j < newColumnsLeft.length; j++) { // for each
				...
				newNode = new Node(j, DataStructureSingleton.getPattern1()[j],
						DataStructureSingleton.getNodeNames()[j], parentNode); // create
																				// new
																				// node
																				// if
																				// it
				...
				System.out.println("Information_Gain for going on branch: {"
						+ FileReader.nodesNames[parentNode.index] + "->"
						+ FileReader.nodesNames[newNode.index] + "}\t= "
			...
			if (nodeHighestInformationGain == null) {
				...
				System.out
						.println("==> Highest Information_Gain is going on branch: {"
								+ FileReader.nodesNames[parentNode.index]
								+ "->"
								+ FileReader.nodesNames[nodeHighestInformationGain.index]
								+ "}\t= "
